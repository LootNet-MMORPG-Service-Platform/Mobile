...
const scrubDebugIds = (value = '') => String(value).replace(
...
const isTwoHanded = (weapon) => {
  ...
  if (typeof t === 'string') {
    ...
    return s.includes('twohand') || s.includes('polearm') || s.includes('bow') || s.includes('crossbow');
  }
  if (typeof t === 'number') {
...
export default function BattleScreen() {
  ...
  const occupiedEnemyPositions = useMemo(() => new Set((battle?.enemies || []).map((e) => e.position)), [battle]);
  const canMoveFront = !!battle && battle.playerPosition < 3 && !occupiedEnemyPositions.has(moveFrontTarget) && !loading;
  const canMoveBack = !!battle && battle.playerPosition > 0 && !occupiedEnemyPositions.has(moveBackTarget) && !loading;

  const weaponOptions = useMemo(() => {
    const keys = ['weapon1', 'weapon2', 'weapon3', 'weapon4', 'Weapon1', 'Weapon2', 'Weapon3', 'Weapon4'];
    const seen = new Set();
    const out = [];
    keys.forEach((k) => {
  ...
  const leftHandLabel = useMemo(
    () => weaponOptions.find((w) => w.id === (battle?.leftHandItemId || battle?.LeftHandItemId))?.name || 'None',
    [weaponOptions, battle]
  );
  const rightHandLabel = useMemo(
    () => weaponOptions.find((w) => w.id === (battle?.rightHandItemId || battle?.RightHandItemId))?.name || 'None',
    [weaponOptions, battle]
  );

  const selectedLeftWeapon = useMemo(() => weaponById.get(selectedLeftHandId) || null, [weaponById, selectedLeftHandId]);
  const selectedRightWeapon = useMemo(() => weaponById.get(selectedRightHandId) || null, [weaponById, selectedRightHandId]);
  ...
  const applyBattleHands = async () => {
    ...
    if (leftWeapon && isTwoHanded(leftWeapon) && (!rightWeapon || rightWeapon.id !== leftWeapon.id)) {
      Alert.alert('Battle hands', 'Two-handed weapon must occupy both hands.');
      return;
    }

    if (rightWeapon && isTwoHanded(rightWeapon) && (!leftWeapon || leftWeapon.id !== rightWeapon.id)) {
      Alert.alert('Battle hands', 'Two-handed weapon must occupy both hands.');
      return;
    }
  ...
  const renderMiniCard = (item) => {
    ...
    return (
      <View key={item.id} style={styles.miniCard}>
        ...
        <Text style={styles.miniStats}>C {fmt(item.cut)} / B {fmt(item.blunt)}</Text>
    ...
    <>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.panel}>
          ...
          <Text style={styles.panelText}>HP: {battle.playerCurrentHp} / {battle.playerMaxHp}</Text>
          ...
          {(battle.enemies || []).map((e, idx) => (
            <View key={e.id} style={styles.enemyRow}>
              ...
              <TouchableOpacity
                ...
                onPress={() => finishTurn({ type: ACTION.ATTACK, targetPosition: e.position }, `Resolving attack on enemy ${idx + 1}...`)}
                disabled={loading}
              >
          ...
          {!handsEditorOpen ? (
            <View style={styles.row}>
              <TouchableOpacity style={[styles.actionBtn, loading && styles.actionBtnDisabled]} onPress={() => setHandsEditorOpen(true)} disabled={loading}>
            ...
            <View style={styles.handsPanel}>
              ...
              <View style={styles.handsSlotsRow}>
                <View style={styles.handSlotCard}>
                  ...
                  <Text style={styles.handSlotValue} numberOfLines={1}>{selectedLeftWeapon?.name || 'Empty'}</Text>
                </View>
                <View style={styles.handSlotCard}>
                  <Text style={styles.handSlotLabel}>RIGHT</Text>
                  <Text style={styles.handSlotValue} numberOfLines={1}>{selectedRightWeapon?.name || 'Empty'}</Text>
                </View>
              </View>

              <Text style={styles.sectionHint}>1H: use L/R. 2H: only 2H. Selecting 1H removes active 2H from both hands.</Text>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalList} contentContainerStyle={styles.horizontalListContent}>
                ...
                {weaponOptions.length === 0 && <Text style={styles.panelText}>No equipped weapon slots found.</Text>}
              </ScrollView>

              <View style={styles.row}>
                <TouchableOpacity
                  style={[styles.actionBtn, (loading || (!selectedLeftWeapon && !selectedRightWeapon)) && styles.actionBtnDisabled]}
          ...
          <View style={styles.row}>
            <TouchableOpacity
              ...
              onPress={() => finishTurn({ type: ACTION.CHANGE_POSITION, targetPosition: moveFrontTarget }, 'Resolving move...')}
            ...
            <TouchableOpacity
              ...
              onPress={() => finishTurn({ type: ACTION.CHANGE_POSITION, targetPosition: moveBackTarget }, 'Resolving move...')}
          ...
          <TouchableOpacity
            ...
            onPress={() => finishTurn({ type: ACTION.END_TURN, targetPosition: battle?.playerPosition ?? 0 }, 'Ending turn...')}
            disabled={loading}
          >
          ...
          {battleRewards.length > 0 && (
            <View style={styles.logBox}>
              ...
              {battleRewards.map((item) => (
                <Text key={item.id || item.Id} style={styles.logLine}>{item.name || item.Name}</Text>
              ))}
            </View>
          )}
          <Text style={styles.panelText}>Status: {runStatusMap[run.status] || run.status || run.Status}</Text>
          <Text style={styles.panelText}>Encounter: {run.battleIndex || run.BattleIndex}</Text>
          <Text style={styles.panelText}>HP: {run.playerCurrentHp || run.PlayerCurrentHp} / {run.playerMaxHp || run.PlayerMaxHp}</Text>

          <View style={styles.row}>
            <TouchableOpacity style={styles.actionBtn} onPress={goFurther} disabled={loading || run.status === 3 || run.Status === 3}>
              <IconSymbol name="chevron.right" size={16} color="#F4E4C1" />
              <Text style={styles.actionText}>{run.status === 3 || run.Status === 3 ? 'Run Lost' : loading ? 'Working...' : 'Go Further'}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.actionBtn, styles.endBtn]} onPress={endRun} disabled={loading}>
...
const styles = StyleSheet.create({
  ...
  headerTopRow: { width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  title: { fontSize: 28, fontWeight: 'bold', color: '#F4E4C1', textTransform: 'uppercase', fontFamily: 'Lato_700Bold' },
  subtitle: { marginTop: 6, color: '#A0826D', fontFamily: 'Lato_400Regular' },
  helpButton: { width: 34, height: 34, borderRadius: 17, borderWidth: 1, borderColor: '#8B7355', alignItems: 'center', justifyContent: 'center', backgroundColor: '#3E2723' },
  helpButtonText: { color: '#F4E4C1', fontSize: 18, fontWeight: '700', fontFamily: 'Lato_700Bold' },
  panel: { margin: 16, backgroundColor: '#3E2723', borderRadius: 10, borderWidth: 1, borderColor: '#8B7355', padding: 16 },
  panelTitle: { color: '#F4E4C1', fontSize: 18, fontWeight: '700', marginBottom: 8, fontFamily: 'Lato_700Bold' },
  ...
  enemyAttackText: { color: '#F4E4C1', fontWeight: '700', fontSize: 11, fontFamily: 'Lato_700Bold' },
  actionBtn: { marginTop: 12, backgroundColor: '#8B7355', borderRadius: 8, paddingVertical: 11, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, flex: 1 },
  actionBtnDisabled: { opacity: 0.45 },
  actionText: { color: '#F4E4C1', fontWeight: '700', fontFamily: 'Lato_700Bold' },
  row: { flexDirection: 'row', gap: 10 },
  endBtn: { backgroundColor: '#654321' },
  logBox: { marginTop: 10, backgroundColor: '#2C1810', borderWidth: 1, borderColor: '#8B7355', borderRadius: 8, padding: 8 },
  logScroll: { maxHeight: 180 },
  logLine: { color: '#A0826D', fontSize: 12, marginBottom: 4, fontFamily: 'Lato_400Regular' },
  back: { marginTop: 4, marginBottom: 4, paddingVertical: 8, flexDirection: 'row', alignItems: 'center' },
  backTxt: { color: '#F4E4C1', marginLeft: 4, fontFamily: 'Lato_400Regular' },
  handsPanel: { marginTop: 8, padding: 8, borderWidth: 1, borderColor: '#8B7355', borderRadius: 8, backgroundColor: '#2C1810' },
  handsSlotsRow: { flexDirection: 'row', gap: 8, marginBottom: 8 },
  handSlotCard: { flex: 1, backgroundColor: '#3E2723', borderColor: '#8B7355', borderWidth: 1, borderRadius: 8, padding: 8 },
  ...
  miniCard: { width: 148, minHeight: 120, backgroundColor: '#3E2723', borderWidth: 1, borderColor: '#8B7355', borderRadius: 8, padding: 8, marginRight: 8, justifyContent: 'center', alignItems: 'center' },
  miniStats: { color: '#A0826D', marginTop: 6, fontSize: 11, textAlign: 'center', fontFamily: 'Lato_400Regular' },
  miniName: { color: '#F4E4C1', marginTop: 4, fontSize: 11, maxWidth: 130, fontFamily: 'Lato_400Regular' },
